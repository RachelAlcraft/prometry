...
def plot_sheet(structures, geos, id1s,id2s):
    ...
    with tabDemo:
        ...
        with cols[1]:
            geos = st.text_input("Geometric parameters",value=geos,help="space delim, 2, 3 or 4 atoms - see help")
        ...
        if len(df_geos.index) > 0:
            ...
            try:
                ...
                iidz1 = ax_colsZ.index(idz1)
                iidx2 = ax_cols.index(idx2)
                iidy2 = ax_cols.index(idy2)
                iidz2 = ax_colsZ.index(idz2)
            except:
                pass
            ...
            with cols[2]:
                z_ax1 = st.selectbox("z-axis (hue) 1",ax_colsZ,index=iidz1)
            with cols[3]:
                x_ax2 = st.selectbox("x-axis 2", ax_cols,index=iidx2)
            with cols[4]:
                y_ax2 = st.selectbox("y-axis 2",ax_cols,index=iidy2)
            with cols[5]:
                z_ax2 = st.selectbox("z-axis (hue) 2",ax_colsZ,index=iidz2)
